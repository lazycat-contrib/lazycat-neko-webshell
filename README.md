...
键盘工具栏里的编辑图标可以打开多行草稿框。草稿按终端分别保留在当前页面内，Herdr 会区分实际窗格；关闭后可以继续编辑，刷新页面后清除。回车只在草稿里换行，点击“插入终端”不额外发送回车，点击“发送并回车”才提交。发送失败会保留草稿，发送中重新打开不会重复提交。对于没有启用多行粘贴的原生终端，多行“插入”会提示改用明确的发送操作。
