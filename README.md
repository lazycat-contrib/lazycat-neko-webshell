...
支持 machine 管理的 Herdr 版本会在「更多 → 管理机器」中提供连接管理窗口。可以从目标登录用户 `~/.ssh/config` 的 Host 中选择，也可以手动填写 SSH 目标，并在添加前测试连接。测试只检查 SSH；点击添加后才会打开终端完成登录、安装确认和 Herdr 初始化。

连接列表使用 label 作为显示名称，支持重命名、启用、停用和移除。移除只删除 Herdr 的连接记录，远端会话继续运行。机器管理通过目标用户的 Herdr CLI 完成；机器和工作区导航继续使用 Herdr 自带的界面，WebShell 不另建常驻机器树。
...
键盘工具栏里的编辑图标可以打开多行草稿框。草稿按终端分别保留在当前页面内，Herdr 会区分实际窗格；关闭后可以继续编辑，刷新页面后清除。回车只在草稿里换行，点击“插入终端”不额外发送回车，点击“发送并回车”才提交。发送失败会保留草稿，发送中重新打开不会重复提交。对于没有启用多行粘贴的原生终端，以及无法确认内部粘贴模式的 Herdr 窗格，多行“插入”会提示改用明确的发送操作。

点击导航分类后，方向键直接显示在下方按键栏的最前面，默认主栏不重复放置方向键。Home、End、翻页和删除等按键排列在后面，支持横向滚动与长按连发；滑动时不会误发送按键。导航与其他分类采用相同的切换方式，不弹出浮层，也不会主动开关系统键盘；自定义键盘布局保持原样。

短语图标打开可搜索的“短语与按键”面板，提供最近使用、全部短语和按键三个分页。短语沿用移动端设置里的内容和“自动回车”选项，面板会明确标出是否回车。操作页中的会话总览显示窗格数量、连接状态和当前选中项。

## 机密文件

需要给 AI 或 agent 提供密钥文件时，PC 按 `Ctrl+Alt+V`（Mac 按 `⌘⌥V`），或在终端右键菜单中选择“机密文件”。手机直接点击键盘菜单栏里的文件密钥图标，不需要切换快捷键页或打开右键菜单。PC 快捷键可在机密文件面板中改成 `Ctrl+Alt+K`（Mac 为 `⌘⌥K`），也可以关闭。

机密文件是内置功能，无需打开工具面板或启用文件传输插件。WebShell 会读取剪贴板文本，在当前终端目标实例的 `/tmp` 下创建随机命名的私有目录和文件。文件权限为 `600`，目录权限为 `700`。读取为空、被拒绝或等待超过 3 秒时，会直接显示输入框供手动粘贴；内容保留原有换行和空格，最大 1 MiB。

生成后的小弹窗会分别显示随机文件名与完整路径，可以插入终端或复制路径，插入时不会自动回车。密钥正文不会发送到终端历史；AI 或 agent 如果读取文件，内容仍可能进入其上下文。面板在当前页面保留已生成的路径，可重新打开、选择和删除；刷新页面会清除这份路径列表，目标文件仍然存在。用完请删除，关闭面板不会删除文件，系统清理时间不固定。
...
在支持只读快照的目标上，多个页面会自动同步标签和窗格的新增、改名与删除，同时保留各自正在查看的位置。同步不会重建未变化的终端，也不会主动恢复已停止的会话。兼容的旧 agent 可以继续使用，暂不启用这项后台同步。

开启设置里的调试模式后，可以复制或下载终端诊断记录，用于查看连接、回放、尺寸变化和重连阶段。记录不包含命令、终端输出或认证信息；关闭调试模式会清除记录。

开发者可以运行 `npm run test:browser` 执行浏览器回归，环境要求和覆盖范围见 [浏览器测试](tests-auto/README.md)。
