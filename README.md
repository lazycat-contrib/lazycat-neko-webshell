...
需要给 AI 或 agent 提供密钥文件时，PC 按 `Ctrl+Alt+V`（Mac 按 `⌘⌥V`），或点击顶部的文件密钥图标。手机直接点击键盘菜单栏里的同款图标，不需要切换快捷键页或打开右键菜单。PC 快捷键可在机密文件面板中改成 `Ctrl+Alt+K`（Mac 为 `⌘⌥K`），也可以关闭。

WebShell 会读取剪贴板文本，在当前终端目标实例的 `/tmp` 下创建随机命名的私有目录和文件。文件权限为 `600`，目录权限为 `700`。读取为空、被拒绝或等待超过 3 秒时，会直接显示输入框供手动粘贴；内容保留原有换行和空格，最大 1 MiB。

生成后可以插入终端或复制路径，插入时不会自动回车。密钥正文不会发送到终端历史；AI 或 agent 如果读取文件，内容仍可能进入其上下文。面板在当前页面保留已生成的路径，可重新打开、选择和删除；刷新页面会清除这份路径列表，目标文件仍然存在。用完请删除，关闭面板不会删除文件，系统清理时间不固定。
