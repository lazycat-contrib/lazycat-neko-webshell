...
pub fn build_app(state: Arc<AppState>) -> Router {
    ...
    Router::new()
        ...
        .route("/api/secret-files", post(crate::secret_file::http::create)
            .delete(crate::secret_file::http::remove)
            .layer(DefaultBodyLimit::max(crate::secret_file::MAX_SECRET_BYTES)))
        ...
        .layer(security_header(
            ...
            "default-src 'self'; connect-src 'self' ws: wss:; font-src 'self' data: blob:; img-src 'self' data:; media-src 'self' blob:; style-src 'self' 'unsafe-inline'; script-src 'self' 'wasm-unsafe-eval'; worker-src 'self'; object-src 'none'; base-uri 'self'",
        ))
}
