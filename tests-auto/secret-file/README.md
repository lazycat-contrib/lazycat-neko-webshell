...
The fixture imports the production controller, view, styles, and mobile keyboard controller. Clipboard and RPC responses use fictional test data; no real credentials or target files are accessed. Checks cover desktop shortcuts, exact bytes, copy fallback inside the modal, insertion without Enter, target changes, reopening stored paths, mobile icon activation, empty/denied/pending clipboard fallback, late responses, and narrow layouts. Screenshots are saved under `tests-auto/artifacts/secret-file/`.
