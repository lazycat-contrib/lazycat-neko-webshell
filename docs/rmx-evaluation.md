...
Keep LazyCat Neko WebShell's current `TerminalRegistry` / `ManagedTerminal` session runtime for this migration. Revisit `rmx` after the Restty frontend is stable and after the integration points below are either supported upstream or wrapped behind a local adapter.
...
- `InProcessBackend` uses `portable-pty`, `tokio`, and broadcast-style session events, matching LazyCat Neko WebShell's current runtime stack.
...
LazyCat Neko WebShell's session layer currently owns several app-specific contracts that are not a one-to-one fit yet:
...
4. Map LazyCat Neko WebShell session metadata into `rmx` metadata/store semantics.
